...
import { DefaultHeaderAction, ResourceAction, RowAction } from '../../../redux/actionButtonsSlice';
...
function ResourceTableContent<RowItem extends KubeObject>(props: ResourceTableProps<RowItem>) {
  ...
  const renderRowActionMenuItems = useMemo(() => {
    ...
    return ({ closeMenu, row }: { closeMenu: () => void; row: MRT_Row<RowItem> }) => {
      ...
      for (const provider of resourceActionProviders) {
        try {
          const res = provider(item as unknown as KubeObject);
          if (res) {
      ...
      const customActionsElements = visibleCustomActions.map(action => {
        const handleExecute = async () => {
          ...
          await dispatch(
            clusterAction(
              async () => {
                await action.action(item as unknown as KubeObject);
              },
              {
                startMessage: `Executing ${action.label}…`,
                successMessage: `Successfully executed ${action.label}.`,
                errorMessage: `Failed to execute ${action.label}.`,
              }
            )
          );
        };
  ...
  }, [actionsProcessed, resourceActionProviders, dispatch]);
